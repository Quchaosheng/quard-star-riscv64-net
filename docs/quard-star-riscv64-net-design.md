...
- S-mode port 使用 supervisor CSR 保存和恢复任务上下文，并通过 SBI TIME 设置下一次 tick；hart 7 不直接访问 CLINT。
- FreeRTOS 调度器必须在多个 timer tick 后输出一次 `QS:TRUSTED_SCHED_OK`。启动调度器之前打印的 ready 标记不能作为调度成功证据。
- PMP 使用 `0xBF800000-0xBFFFFFFF` 的 8 MiB 对齐区域保护 trusted 代码和数据；hart 7 只额外获得 UART2 的轮询 MMIO 权限。
- 普通内核映射 trusted 物理页后执行受控读取，必须收到 load access fault 并输出 `QS:PMP_UNTRUSTED_DENY_OK`。
- trusted FreeRTOS 对普通内核 DRAM 执行受控读取，必须收到 load access fault 并输出 `QS:PMP_TRUSTED_DENY_OK`。
- M8 smoke 必须同时检查调度器和双向拒绝标记，并继续通过原有七 hart 网络与文件传输验收。
- 在上述运行时证据全部通过前，不得宣称 PMP 隔离完成或发布 `v1.0.0`。
...
M0-M9 已按本设计实施并随 `v1.0.0` 发布。后续变更应保持每个
改动可独立构建和验收，并在合并前完成以下工作：

1. 在 Ubuntu 24.04 或 26.04 运行 `make test-host`。
2. 对影响固件、驱动或网络协议的变更运行相应的 QEMU/TAP smoke
   测试。
3. 更新 README、构建调试指南、限制说明和第三方依赖记录中的受影响
   内容。
4. 将新能力、优化或平台支持明确设计为后续版本范围，不倒改 1.0
   已验证的验收边界。
