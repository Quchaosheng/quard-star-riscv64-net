...
README 中的视频是一次真实 M8 QEMU/TAP 验收运行的紧凑回放。它不会伪造
成功结果：渲染器首先校验普通内核日志、可信 UART2 日志和 TAP 对端统计，
之后才把通过验收的证据制作成可读的 42 秒视频。
...
视频是解释性产物，不是额外的验收权威。原始日志、对端统计、冒烟测试退出
状态和 CI 产物仍是权威证据。

定时、手动和发布标签触发的 M8 工作流会在冒烟验收通过后生成相同媒体。
重新生成的 MP4、海报、证据 JSON、串口日志和对端统计会一起上传到
