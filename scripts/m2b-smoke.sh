...
has_success_markers() {
  grep -q 'QS:BOOT_OK' "$combined" 2>/dev/null &&
    ...
    grep -q 'QS:TEST_PASS:m2a-smoke' "$combined" 2>/dev/null &&
    grep -q 'QS:SMP_SCHED_OK' "$combined" 2>/dev/null &&
    grep -q 'QS:TEST_PASS:m2b-smoke' "$combined" 2>/dev/null
}

for file in "$qemu" "$out/fw/fw.bin" "$out/disk/disk.img"; do
  if [ ! -e "$file" ]; then
    echo "error: missing $file; run make m2b-build" >&2
    exit 1
  fi
done
...
if [ "$success" -ne 1 ]; then
  for marker in QS:BOOT_OK QS:KERNEL_READY QS:BLOCK_OK \
    QS:HART_ONLINE:0 QS:HART_ONLINE:1 QS:SMP_ALLOC_OK \
    QS:TEST_PASS:m2a-smoke QS:SMP_SCHED_OK \
