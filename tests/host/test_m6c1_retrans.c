...
static void establish(tcp_pcb_t **result, netif_t *netif,
                      const ipaddr_t *remote, uint16_t port)
{
    assert(tcp_open(result) == NET_ERR_OK);
    tcp_pcb_t *pcb = *result;
...
{
    tcp_pcb_t *pcb = 0;
    recv_thread_arg_t arg = { .pcb = 0, .result = 0 };
    pthread_t thread;
...
static void test_peer_first_close(netif_t *netif, const ipaddr_t *remote)
{
    ...
    assert(pcb->state == TCP_STATE_FIN_WAIT_1);
    assert(pcb->peer_fin_seen);
    assert(last_output_flags == (TCP_FLAG_FIN | TCP_FLAG_ACK));
    input_ok(netif, remote, &netif->ipaddr,
             make_segment(remote, &netif->ipaddr, 4807, pcb->local_port,
                          pcb->rcv_nxt, pcb->snd_nxt, TCP_FLAG_ACK, 0, 0));
    assert(pcb->state == TCP_STATE_TIME_WAIT);
