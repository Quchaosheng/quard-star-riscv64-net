...
{
    return buf != 0 ? buf->total_size - buf->pos : 0;
}
...
{
    if (buf == 0 || buf->curr_blk == 0 || buf->blk_offset == 0)
        return 0;

    return (int)(buf->curr_blk->data + buf->curr_blk->size -
                 buf->blk_offset);
}
...
static void display_check_buf(pktbuf_t *buf)
{
    ...
    if (total_size != buf->total_size) {
        dbg_error(DBG_BUF, "bad buf size. %d != %d", total_size,
                  buf->total_size);
    }
}
#else
                                   ...
                                   int add_last)
{
    if (add_last) {
        ...
        while (first_blk) {
            ...
            if (pre) {
                nlist_insert_after(&buf->blk_list, &pre->node,
                                   &first_blk->node);
            } else {
...
net_err_t pktbuf_add_header(pktbuf_t *buf, int size, int cont)
{
    ...
    if (cont) {
        ...
        if (size > PKTBUF_BLK_SIZE) {
            dbg_error(DBG_BUF, "is_contious && size too big %d > %d", size,
                      PKTBUF_BLK_SIZE);
            return NET_ERR_SIZE;
        }
        ...
        int extra_size = size - resv_size;
        pktblk_t *new_block = pktblock_alloc_list(extra_size, 1);
        if (new_block == 0) {
            dbg_error(DBG_BUF, "no buffer for alloc(size:%d)", size);
            return NET_ERR_MEM;
        }
...
net_err_t pktbuf_resize(pktbuf_t *buf, int to_size)
{
    ...
    if (to_size == 0) {
        pktblk_t *block;
        while ((block = pktbuf_first_blk(buf)) != 0) {
            nlist_remove_first(&buf->blk_list);
            pktblock_free(block);
        }
        buf->total_size = 0;
...
net_err_t pktbuf_set_cont(pktbuf_t *buf, int size)
{
    ...
    while (remain_size && curr_blk) {
        // 计算本次移动的数据量
        int curr_size = (curr_blk->size >
                         remain_size) ? remain_size : curr_blk->size;
...
net_err_t pktbuf_init(void)
{
    ...
    net_err_t err = mblock_init(&block_list, block_buffer, sizeof(pktblk_t),
                                PKTBUF_BLK_CNT, NLOCKER_NONE);
    if (err < 0)
        return err;
    err = mblock_init(&pktbuf_list, pktbuf_buffer, sizeof(pktbuf_t),
                      PKTBUF_BUF_CNT, NLOCKER_NONE);
    if (err < 0)
        return err;
                           ...
                           int complement)
{
    ...
    int remain_size = total_blk_remain(buf);
    if (remain_size < size) {
        dbg_warning(DBG_BUF, "size too big. sum size %d < remain size: %d",
                    size, remain_size);
        return 0;
    }
    ...
    while (size > 0) {
        ...
        dbg_assert(buf->blk_offset + curr_size <=
                   buf->curr_blk->data + PKTBUF_BLK_SIZE, "out bound");
